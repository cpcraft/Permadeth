...
const state = {
  ...
  loot: new Map(),      // id -> loot
  sprites: new Map(),   // id -> { g, visX, visY, tagEl? }
  lootSprites: new Map(), // id -> PIXI.Graphics
  ws: null,
...
const indicatorLayer = new PIXI.Container(); // local-only fx
...
/* ==============================
   RIGHT-CLICK TO MOVE + CROSSHAIR
============================== */
app.stage.eventMode = 'static';
app.stage.hitArea = app.screen;
canvas.addEventListener('contextmenu', (e) => e.preventDefault()); // allow right-click
...
btnRegister.onclick = async () => {
  try {
    ...
    logChat('[Auth] Registration error');
  }
};
    ...
    switch (t) {
      ...
      case 'PLAYER_MOVES':
        d.forEach(({ id, x, y }) => {
          ...
          p.x = x; p.y = y;
          if (wasMe) centerCameraNow();  // camera follows raw server pos (no smoothing)
        });
        break;
...
function ensurePlayerSprite(p) {
  ...
  state.sprites.set(p.id, entry);   // ✅ KEYED BY p.id (bug fix)
  world.addChild(g);
}
...
function updatePlayersList() {
  const ts = state.constants?.TILE_SIZE || 64;
  const rows = [];
  for (const p of state.players.values()) {
    rows.push(`${p.name}  ${Math.floor(p.x / ts)}:${Math.floor(p.y / ts)}`);
  }
  rows.sort();
...
function send(op, d = {}) {
  if (state.ws && state.ws.readyState === 1) state.ws.send(JSON.stringify({ op, d }));
}
...
function centerCameraNow() {
  ...
  const ts = state.constants?.TILE_SIZE || 64;
  coordsEl.textContent = `x:${Math.floor(me.x / ts)}  y:${Math.floor(me.y / ts)}`;
  updatePlayersList();
}
...
app.ticker.add(() => {
  ...
  const alpha = 1 - Math.exp(-10 * dt); // smoothing factor
  for (const [id, p] of state.players) {
    ...
    if (s.tagEl) {
      s.tagEl.style.left = `${s.visX + world.x}px`;
      s.tagEl.style.top = `${s.visY + world.y - 24}px`;
    }
  }
...
/* ==============================
   CHUNKED TILEMAP (permanent feature)
============================== */
const tileRenderer = (() => {
  ...
  function tileColor(type, tx, ty) {
    ...
    if (type === 'grass') {
      const b = [0x2a, 0x7d, 0x3b];
      return ((Math.min(255, b[0] + v * 40) << 16) |
              (Math.min(255, b[1] + v * 40) << 8) |
               Math.min(255, b[2] + v * 20)) >>> 0;
    }
    const b = [0x7a, 0x5c, 0x3b];
    return ((Math.min(255, b[0] + v * 30) << 16) |
            (Math.min(255, b[1] + v * 25) << 8) |
             Math.min(255, b[2] + v * 20)) >>> 0;
  }
  function drawChunk(cx, cy) {
    const gfx = new PIXI.Graphics();
    const ox = cx * chunkSizeTiles * tileSize;
    const oy = cy * chunkSizeTiles * tileSize;
    for (let ty = 0; ty < chunkSizeTiles; ty++) {
      for (let tx = 0; tx < chunkSizeTiles; tx++) {
        ...
        const wx = ox + tx * tileSize;
        const wy = oy + ty * tileSize;
  ...
  function ensureChunk(cx, cy) {
    ...
    const spr = drawChunk(cx, cy);
    chunks.set(k, { sprite: spr });
  ...
  function cull(view) {
    for (const [k, obj] of chunks) {
      ...
      const margin = w * 1.5;
      const inView = !(x + w < view.x - margin ||
  ...
  return {
    init({ tileSize: ts, worldTiles: wt }) { tileSize = ts; worldTiles = wt; },
    ensureChunks(cx, cy, vw, vh) {
      const hw = Math.ceil(vw / 2), hh = Math.ceil(vh / 2);
...
function showClickCrosshair(x, y) {
  ...
  crosshair.visible = true; crosshair.alpha = 1;
...
function updateCrosshair(dt) {
  ...
  // fade after 3 seconds, hide after 4
